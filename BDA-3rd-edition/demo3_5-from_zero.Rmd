...
df1$pm_mu <- dt((df1$t1 - mean_y) / sqrt(s2/n), n-1) / sqrt(s2/n)
```

Compute the exact marginal posterior density for mu for the positive data

```{r }
df1$pm_mu_pos = dt((df1$t1 - mean_y_pos) / sqrt(s2_pos/n_pos), n_pos-1) / sqrt(s2_pos/n_pos)
```
